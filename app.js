document.addEventListener("DOMContentLoaded", function () {
  ...
  function startNewGame() {
    try {
      ...
      alert("Sorry, we couldn't generate a puzzle. Trying an easy one instead.");
  ...
  function generatePuzzleFast(targetDifficulty) {
      ...
      const cellsToKeep = Math.floor(
          Math.random() * (targetCells.max - targetCells.min + 1)
      ) + targetCells.min;
      
      const solution = generateSolvedBoard();
      const board = removeNumbersFast(JSON.parse(JSON.stringify(solution)), 81 - cellsToKeep);
 ...
 // ======================
    ...
    function verifyDifficulty(board, targetDifficulty) {
      ...
      return Math.max(difficultyValue(estimatedDiff), difficultyValue(targetDifficulty)) === difficultyValue('insane') 
          ? 'insane' 
          : estimatedDiff;
  }
...
// ======================
  ...
  function getCandidateCols(board, row, num) {
      const cols = [];
      for (let col = 0; col < 9; col++) {
          if (board[row][col] === 0 && getCandidates(board, row, col).includes(num)) {
              cols.push(col);
          }
      }
      return cols;
  ...
  function updateBestTime() {
      ...
      if (!currentBest || seconds < parseInt(currentBest)) {
          localStorage.setItem(`sudoku-best-time-${difficulty}`, seconds.toString());
