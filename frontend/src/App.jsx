...
import {
  loadRemoteStats, saveRemoteStats, getRankForTime, insertGameSession,
...
export default function App() {
  ...
  const handleComplete = useCallback(async ({ difficulty: diff, time, hintsUsed }) => {
    ...
    // Save updated stats + record the session (run in parallel for speed)
    await Promise.all([
      ...
      insertGameSession(auth.user.id, diff, time, hintsUsed),
    ]);
